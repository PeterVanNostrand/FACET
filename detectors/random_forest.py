...
class RandomForest(Detector):
    ...
    def get_candidate_examples_treewise(self, x, y):
        ...
        trees = self.model.estimators_
        all_examples = np.empty(shape=(len(trees), x.shape[0], x.shape[1]))

        # for each tree, get an example for each sample in x
        for i in range(self.ntrees):
            # get an array of candidate examples for each instance in x
            helper = TreeContraster(trees[i])
            tree_candidates = helper.construct_examples(x, y, self.difference)
            ...
            for j in range(x.shape[0]):
                if tree_candidates[j].shape[0] > 0:
                    best_candidate_idx = np.argmin(self.distance_fn(x[j], tree_candidates[j]))
                    tree_best[j] = tree_candidates[j][best_candidate_idx]
                else:
                    tree_best[j] = np.tile(np.nan, x.shape[1])

            all_examples[i] = tree_best
