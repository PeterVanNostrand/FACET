    ...
    useEffect(() => {
        ...
        try {
            ...
            const newFeatures = Object.entries(formatDict.feature_names).map(([key, value], index) => {
                ...
                lower_constraint = Math.min(lower_constraint, (current_value - (current_value * 0.2)));
                lower_constraint = Math.max(lower_constraint, line_min);
                let upper_constraint = (line_min + (line_max - line_min) * 0.9);
                upper_constraint = Math.max(upper_constraint, (current_value + (current_value * 0.2)));
                lower_constraint = Math.min(lower_constraint, line_max);
