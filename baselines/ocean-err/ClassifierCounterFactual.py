...
class ClassifierCounterFactualMilp:
    def __init__(
        ...
        self.discreteAndCategoricalNonOneHotFeatures = [i for i in range(self.nFeatures) if self.featuresType[i] in [
    ...
    def initObjectiveOfFeature(self, f):
        if self.featuresType[f] == FeatureType.CategoricalNonOneHot:
            ...
            self.absoluteValueRightConstr[f] = self.model.addConstr(
                ...
                self.x_var_sol[f] + self.x0[0][f], "absoluteValueRightConstr_f"+str(f)
            )
