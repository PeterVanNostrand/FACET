        ...
        for t in range(self.completeForest.n_estimators):
            ...
            while len(stack) > 0:
                ...
                if cr != cl:
                    ...
                    self.nodeAncestorsAlongFeature_tvf[t][cl] = {
                        f: self.nodeAncestorsAlongFeature_tvf[t][v][f].copy() for f in self.nodeAncestorsAlongFeature_tvf[t][v]}
                    ...
                    self.nodeAncestorsAlongFeature_tvf[t][cr] = {
                        f: self.nodeAncestorsAlongFeature_tvf[t][v][f].copy() for f in self.nodeAncestorsAlongFeature_tvf[t][v]}
            ...
            for leaf in self.leafVar[t]:
                ...
                self.model.addConstr(self.leafVar[t][leaf] <= upperBound,
                                     "consistency_tree" + str(t) + "_leaf" + str(leaf))
            ...
            for f in self.cellVar:
                if self.featuresType[f] == FeatureType.Numeric:
                    for cell in self.cellVar[f]:
                        if self.cellUpperBound[f][cell] < self.x0[0][f]:
                            self.nearestInCell[f][cell] = self.cellUpperBound[f][cell] - eps
                            obj += (self.x0[0][f] - self.nearestInCell[f][cell]) * (self.x0[0]
                                                                                    [f] - self.nearestInCell[f][cell]) * self.cellVar[f][cell]
                        elif self.cellLowerBound[f][cell] > self.x0[0][f]:
                            self.nearestInCell[f][cell] = self.cellLowerBound[f][cell] + eps
