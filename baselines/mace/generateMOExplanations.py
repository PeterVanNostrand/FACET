...
def findClosestObservableSample(observable_samples, dataset_obj, factual_sample, norm_type):
    ...
    # in case no observables are found.
    closest_observable_sample = {'sample': {}, 'distance': np.infty, 'norm_type': None}
    ...
    for observable_sample_index, observable_sample in observable_samples.items():
        ...
        observable_distance = normalizedDistance.getDistanceBetweenSamples(
            factual_sample, observable_sample, norm_type, dataset_obj)

        if violating_actionable_attributes:
            continue

        if observable_distance < closest_observable_sample['distance']:
            closest_observable_sample = {'sample': observable_sample,
                                         'distance': observable_distance, 'norm_type': norm_type}
...
def genExp(
    ...
    print(
        f"Best observable sample: \t {getPrettyStringForSampleDictionary(closest_observable_sample['sample'], dataset_obj)} (verified)", file=log_file)
    print('', file=log_file)
    print(
        f"Minimum observable distance (by searching the dataset):\t {closest_observable_sample['distance']:.6f}", file=log_file)
