...
class Dataset(object):
    ...
    def __init__(self, data_frame, attributes, is_one_hot, dataset_name):
        ...
        for attr_name in np.setdiff1d(
            ...
            if is_one_hot and attributes_long[attr_name].attr_type != 'numeric-int':
                try:
                    assert \
        ...
        if long_or_kurz == 'long':
            ...
            for key in dict_of_siblings_long['cat'].keys():
                dict_of_siblings_long['cat'][key] = sorted(
                    dict_of_siblings_long['cat'][key], key=lambda x: int(x.split('_')[-1]))

            for key in dict_of_siblings_long['ord'].keys():
                dict_of_siblings_long['ord'][key] = sorted(
                    dict_of_siblings_long['ord'][key], key=lambda x: int(x.split('_')[-1]))
            ...
            for key in dict_of_siblings_kurz['cat'].keys():
                dict_of_siblings_kurz['cat'][key] = sorted(
                    dict_of_siblings_kurz['cat'][key], key=lambda x: int(x.split('_')[-1]))

            for key in dict_of_siblings_kurz['ord'].keys():
                dict_of_siblings_kurz['ord'][key] = sorted(
                    dict_of_siblings_kurz['ord'][key], key=lambda x: int(x.split('_')[-1]))
    ...
    def getBalancedDataFrame(self):
        ...
        number_of_subsamples_in_each_class = unique_values_and_count.min() // 100 * 100  # // 250 * 250
    ...
    def __init__(
        ...
        if attr_type in {'binary', 'categorical', 'sub-categorical'}:  # not 'ordinal' or 'sub-ordinal'
            ...
            assert actionability in {
                'none', 'any'}, f"{attr_type}'s actionability can only be in {'none', 'any'}, not `{actionability}`."
...
def loadDatasetMine(df, dataset_name, return_one_hot, load_from_cache=False, debug_flag=True, meta_param=None):
    ...
    if dataset_name == 'dataset':
        ...
        attributes_non_hot[col_name] = DatasetAttribute(
            ...
            parent_name_long=-1,
            parent_name_kurz=-1,
            lower_bound=0.0,  # data_frame_non_hot[col_name].min(),
            upper_bound=1.0  # data_frame_non_hot[col_name].max()
        )
...
def loadDataset(dataset_name, return_one_hot, load_from_cache=False, debug_flag=True, meta_param=None, my_df=None):
    ...
    elif dataset_name == 'cust_data':

        data_frame_non_hot = my_df
        data_frame_non_hot = data_frame_non_hot.reset_index(drop=True)
        attributes_non_hot = {}
