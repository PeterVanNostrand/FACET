        ...
        for model_class_string in MODEL_CLASS_VALUES:
            ...
            for norm_type_string in NORM_VALUES:
                ...
                for approach_string in APPROACHES_VALUES:
                    ...
                    try:
                        assert len(
                            matching_child_folders) == 1, f'Expecting only 1 folder, but we found {len(matching_child_folders)}.'
                        ...
                        print(
                            f'\t[{counter} / (max {all_counter})] Cannot find folder for {dataset_string}-{model_class_string}-{norm_type_string}-{approach_string}')
                        continue
                    ...
                    try:
                        ...
                        print(
                            f'\t[{counter} / (max {all_counter})] Successfully found folder {matching_child_folder.split("/")[-1]}, found min dist file, ', end='')
                    ...
                    for key in minimum_distance_file.keys():
                        ...
                        if 'MACE' in approach_string:
                            all_counterfactual_distances = list(
                                map(lambda x: x['counterfactual_distance'], minimum_distance_file[key]['all_counterfactuals']))
                            all_counterfactual_times = list(
                                map(lambda x: x['time'], minimum_distance_file[key]['all_counterfactuals']))
                        else:
        ...
        for norm_type_string in NORM_VALUES:
            ...
            for dataset_string in DATASET_VALUES:
                ...
                for approach_string in APPROACHES_VALUES:
                    ...
                    print(
                        f'[INFO] (#{counter} / {total_counter}) Processing {dataset_string}-{model_class_string}-{norm_type_string}-{approach_string}...')
                    counter = counter + 1
                    ...
                    if df.shape[0]:  # if any tests exist for this setup
                        ...
                        if 'MACE' in approach_string:
                            ...
                            max_iterations = max(
                                list(map(lambda x: len(x), df_all_distances['all counterfactual times'])))
                            # print(f'max_iterations: {max_iterations}')

                            for index, row in df.iterrows():

                                # remove the first elem (np.infty)
                                all_counterfactual_distances = row['all counterfactual distances'][1:]
                                # remove the first elem (np.infty)
                                all_counterfactual_times = row['all counterfactual times'][1:]
            ...
            for norm_type_string in NORM_VALUES:
                ...
                if df.shape[0]:  # if any tests exist for this setup
                    ...
                    for approach_string in all_but_mace_approaches:
                        ...
                        assert len(
                            factual_sample_index_intersect) >= MIN_SAMPLES_REQUIRED, f'Expecting at least {MIN_SAMPLES_REQUIRED} intersecting samples between MACE and {approach_string}'
                        ...
                        for factual_sample_index in factual_sample_index_intersect:
                            ...
                            assert len(
                                sample_mace.keys()) == 1, f'Expecting only 1 sample with index {factual_sample_index} for approach {approach_string}'
                            sample_other = df.where(
                                (df['approach'] == approach_string) &
                                (df['factual sample index'] == factual_sample_index)
                            ).dropna().T.to_dict()
                            assert len(
                                sample_other.keys()) == 1, f'Expecting only 1 sample with index {factual_sample_index} for approach {approach_string}'
                            minimum_distance_mace = sample_mace[list(sample_mace.keys())[0]]['counterfactual distance']
                            minimum_distance_other = sample_other[list(sample_other.keys())[
                                0]]['counterfactual distance']
                        ...
                        print(
                            f'\t Distance reduction for {dataset_string} {model_class_string} {norm_type_string} (1 - d_MACE / d_{approach_string}) = \t {tmp_mean:.2f} +/- {tmp_std:.2f} \t (N = {len(distance_reduction_list)})')
        ...
        for approach_string in APPROACHES_VALUES:
            for dataset_string in DATASET_VALUES:
                for norm_type_string in NORM_VALUES:
                    ...
                    if df.shape[0]:  # if any tests exist for this setup
                        found_and_plausible = df.where((df['counterfactual found'] == True) & (
                            df['counterfactual plausible'] == True))
                        found_and_not_plausible = df.where(
                            (df['counterfactual found'] == True) & (df['counterfactual plausible'] == False))
                        ...
                        assert df.shape[0] == \
                            ...
                            count_not_found
                        # this is NOT a good way to compare methods! see analyzeRelativeDistances() instead, as it compares ratio of distances for the same samples!
                        average_distance = found_and_plausible['counterfactual distance'].mean()
                        ...
                        print(
                            f'{model_class_string}-{approach_string}-{dataset_string}-{norm_type_string} ({count_found_and_plausible} plausible samples found):')
...
def plotAvgDistanceRunTimeCoverageTradeoffAgainstIterations():
    ...
    for model_class_string in MODEL_CLASS_VALUES:
        ...
        approach_order = [r'MACE ($\epsilon = 10^{-2}$)',
                          r'MACE ($\epsilon = 10^{-3}$)', r'MACE ($\epsilon = 10^{-5}$)']
        ...
        for norm_type_string in NORM_VALUES:
            ...
            fig.tight_layout()
            fig.savefig(
                f'_results/{tmp_constrained}__avg_tradeoff__{model_class_string}_{norm_type_string}.png', dpi=300)
