...
import baselines.mace.utils as utils
...
try:
    import baselines.mace.treeUtils as treeUtils
except:
...
def loadModelForDataset(model_class, dataset_string, scm_class=None, experiment_folder_name=None):
    ...
    if not (dataset_string in {'synthetic', 'mortgage', 'twomoon', 'german', 'credit', 'compass', 'adult', 'test', 'vertebral', 'magic'}):
    ...
    feature_names = dataset_obj.getInputAttributeNames('kurz')

    if model_class == 'tree':
        model_pretrain = DecisionTreeClassifier()
    elif model_class == 'forest':
        model_pretrain = RandomForestClassifier(n_estimators=10, max_depth=5)
        print("Forest: n_estimators={}, max_depth={}".format(model_pretrain.n_estimators, model_pretrain.max_depth))
    elif model_class == 'lr':
    ...
    if model_class == 'tree':
        ...
        for tree_idx in range(len(model_trained.estimators_)):
            if SIMPLIFY_TREES:
                print(
                    f'[INFO] Simplifying decision tree (#{tree_idx + 1}/{len(model_trained.estimators_)})...', end='', file=log_file)
                model_trained.estimators_[tree_idx].tree_ = treeUtils.simplifyDecisionTree(
...
def scatterDataset(dataset_obj, classifier_obj, ax):
    ...
    for idx in range(number_of_samples_to_plot):
        ...
        if X_train.shape[1] == 2:
            ...
            ax.scatter(X_train_numpy[idx, 0], X_train_numpy[idx, 1], X_train_numpy[idx, 2],
                       marker='s', color=color_train, alpha=0.2, s=10)
            ax.scatter(X_test_numpy[idx, 0], X_test_numpy[idx, 1], X_test_numpy[idx, 2],
                       marker='o', color=color_test, alpha=0.2, s=15)
