...
from utilities.metrics import average_distance, percent_valid
...
def execute_run(model: MethodManager, xtrain, xtest, ytrain, ytest):
    '''
    ...
    coverage_ratio = percent_valid(explanations)
    ...
    for ds_name in ["thyroid", "cardio", "wbc", "musk"]:
        ...
        for diff in differences:
            ...
            for i in range(num_iters):
                ...
                model = MethodManager(detectors=dets, aggregator=agg, explainer=expl, hyperparameters=params)
    ...
    for ds in ds_names:
        ...
        for n in ntrees:
            ...
            for i in range(num_iters):
                ...
                model = MethodManager(detectors=dets, aggregator=agg, hyperparameters=params)
        ...
        for i in range(num_iters):
            ...
            for n in ntrees:
                for d in depths:
                    ...
                    model = MethodManager(detectors=dets, aggregator=agg, hyperparameters=params, explainer=explainer)
                    model.train(xtrain, ytrain)
                    preds = model.predict(xtest)
...
def hard_vs_soft(ds_names, num_iters=5, test_size=0.2, ntrees=20, max_depth=3, seed=None):
    ...
    for ds in ds_names:
        for i in range(num_iters):
            ...
            # Create, train, and predict with the model
            model = MethodManager(detectors=["RandomForest"], aggregator="NoAggregator",
