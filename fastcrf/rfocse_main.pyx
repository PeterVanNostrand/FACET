...
from .debug cimport LOG_LEVEL, ExtractionProblem_tostr, ExtractionContext_tostr, Solution_tostr, instance_num
...
def batch_extraction(sklearn_rf, dataset_info, X, max_distance=-1,
    ...
    if LOG_LEVEL == "DEBUG":
        print("creating batch log")
        f = open("logs/log.txt".format(instance_num), "a")
...
cdef extract_single_counterfactual_set(ExtractionProblem & problem, sklearn_rf, parsed_rf, dataset_info, to_explain, dataset):
    ...
    cdef Solution solution = Solution(
        cppvector[SplitPoint](), # cppvector[SplitPoint] conditions
        cppvector[int](), # cppvector[int] set_rules
